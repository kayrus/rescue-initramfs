...
STATIC_IP=$(ifconfig ${DEV} | awk -F'[: ]+' "/inet /{print \"ip=\"\$4\"::${GW}:\"\$8\":${HOST}:eth0:off\"}")
#STATIC_IP="ip=${IP}::${GW}:${NET}:${HOST}:${DEV}:off"

if [ "${SUDO_USER}" != "" ]; then
  USER_HOME=$(eval echo ~${SUDO_USER})
  PUB_KEY=$(head -1 ${USER_HOME}/.ssh/authorized_keys)
else
  PUB_KEY=$(head -1 /root/.ssh/authorized_keys)
fi
...
echo "The following public key will be used:"
echo "${PUB_KEY}"
...
apt-get update
apt-get install -y busybox-static dropbear lvm2 qemu-utils curl mdadm screen rsync lsof
...
# Ubuntu trusty doesn't support ECDSA
...
rm -f /etc/default/grub.d/99-break-mount.cfg
#reboot
